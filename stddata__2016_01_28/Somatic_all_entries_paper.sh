...
file=$(ls -l $1 | awk -v VAR=$k '{if(NR==VAR) print $9}')

id=$(ls -l $1 | awk -v VAR=$k '{split($9,a,"."); split(a[1],b,"-"); if(NR==VAR) print b[1]"-"b[2]"-"b[3]}')
...
#ruby -n -e 'x=$_.split(/\t/); puts"#{ENV["ID"]}\t#{$_}" if ($.>1)' dummy2.txt >> $out
